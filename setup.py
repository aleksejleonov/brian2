...
if WITH_CYTHON or not os.path.exists(cpp_fname):
    ...
    if not cython_available:
        if FAIL_ON_ERROR and WITH_CYTHON:
            raise RuntimeError(
                "Compilation with Cython requested/necessary but "
...
if fname is not None:
    if (
        ...
        extensions = [
            Extension("brian2.synapses.cythonspikequeue", [fname], include_dirs=[])
        ]  # numpy include dir will be added later
    if fname == pyx_fname:
    ...
    def build_extension(self, ext):
        ...
        try:
            ...
            if FAIL_ON_ERROR:
                ...
                error_msg = (
                    "Building %s failed (see error message(s) "
...
setup(
    ...
    package_data={  # include template files
        ...
        # include randomkit
        "brian2.random": ["randomkit/randomkit.c", "randomkit/randomkit.h"],
    },
    install_requires=[
        "numpy>=1.17",
        "cython>=0.29",
    ...
    setup_requires=["numpy>=1.10", "setuptools>=24.2"],
