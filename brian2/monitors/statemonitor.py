        ...
        if not self.record_all:
            try:
                if len(record) and (
                    ...
                    error_message = (
                        f"The indices to record from contain values outside of the range "
                        f"[0, {len(source)-1}] allowed for the group '{source.name}'"
                    )
                    raise IndexError(error_message)
            except NotImplementedError:
                logger.warn(
                    "Cannot check whether the indices to record from are valid. This can happen "
    ...
    def __getitem__(self, item):
        ...
        if np.issubdtype(dtype, np.signedinteger):
            ...
            if not np.issubdtype(index_array.dtype, np.signedinteger):
                raise TypeError(
                    "Index has to be an integer or a sequence " "of integers"
                )
            return StateMonitorView(self, item)
