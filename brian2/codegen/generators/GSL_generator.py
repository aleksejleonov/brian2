...
class GSLCodeGenerator(object):
    ...
    def scale_array_code(self, diff_vars, method_options):
        ...
        if not isinstance(abs_default, float):
            raise TypeError(
                f"The absolute_error key in method_options should be "
        ...
        if abs_per_var is None:
            ...
            for var, error in list(abs_per_var.items()):
                ...
                if not var in diff_vars:
                    if not var in self.variables:
                        raise KeyError(
                            f"absolute_error specified for variable that "
                            f"does not exist: {var}"
                        )
                    else:
                        raise KeyError(
                            f"absolute_error specified for variable that is "
            ...
            raise TypeError(
                f"The absolute_error_per_variable key in method_options "
    ...
    def translate_vector_code(self, code_lines, to_replace):
        ...
        if "_gsl" in code:
            raise AssertionError(
                f"Translation failed, _gsl still in code (should only "
    ...
    def translate(
        ...
        if any([var in self.variables for var in reserved_variables]):
            ...
            raise ValueError(
                f"The variables {str(reserved_variables)} are reserved for the GSL internal code."
            )
        ...
        if len(self.variables_to_be_processed) > 0:
            raise AssertionError(
                f"Not all variables that will be used in the vector "
    ...
    def unpack_namespace_single(self, var_obj, in_vector, in_scalar):
        ...
        if isinstance(var_obj, ArrayVariable):
            ...
            if in_vector:
                code += [
                    (
                        f"_GSL_dataholder.{array_name} = <{dtype} *> "
                        f"_buf_{array_name}.data"
                    )
                ]
            if in_scalar:
                code += [(f"{array_name} = <{dtype} *> " f"_buf_{array_name}.data")]
        else:
            if in_vector:
                code += [
