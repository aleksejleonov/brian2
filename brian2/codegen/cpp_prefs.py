    ...
    if flags is None:  # If we don't have stored info, run get_cpu_flags.py
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f'Writing file "{flag_file}" to store CPU flags failed with error "{str(ex)}".'
                )
        except subprocess.CalledProcessError as ex:
            logger.debug(
                f"Could not determine optimized MSVC flags, get_cpu_flags failed with: {str(ex)}"
            )
...
def _determine_flag_compatibility(compiler, flagname):
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            logger.warn(
                f"Removing unsupported flag '{flagname}' from " f"compiler flags."
            )
            return False
    return True
