...
class CPPStandaloneDevice(Device):
            ...
            if self.has_been_run:
                ...
                if isinstance(var.size, tuple) and len(var.size) == 2:
                    if var.size[0] * var.size[1] == len(data):
                        ...
                        raise IndexError(
                            f"Do not now how to deal with 2d "
        ...
        for func, args in self.main_queue:
            if func == "before_run_code_object":
                ...
                if seed is None:  # random
                    ...
                    main_lines.append(
                        f"    rk_seed({seed!r}L + _i, brian::_mersenne_twister_states[_i]);"
                    )
            else:
                ...
                with std_silent(debug):
                    if vcvars_cmd:
                        if clean:
                            ...
                            os.system(
                                f"{vcvars_cmd} >>winmake.log 2>&1 && {make_cmd} clean > NUL 2>&1"
                            )
                            self.timers["compile"]["clean"] = time.time() - start_time
                        start_time = time.time()
                        x = os.system(
                            f"{vcvars_cmd} >>winmake.log 2>&1 && {make_cmd} {make_args}>>winmake.log 2>&1"
                        )
                    ...
                    if x != 0:
                        if os.path.exists("winmake.log"):
                            with open("winmake.log", "r") as f:
                                print(f.read())
                        error_message = (
                            "Project compilation failed (error " "code: %u)."
                        ) % x
                        if not clean:
                            error_message += (
    ...
    def build(
        ...
        **kwds,
        ...
        logger.diagnostic(
            f"Writing C++ standalone project to directory "
        ...
        if len(non_unique_names):
            ...
            raise ValueError(
                f"All objects need to have unique names in "
