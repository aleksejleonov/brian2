...
class StateUpdateMethod(object, metaclass=ABCMeta):
    ...
    def register(name, stateupdater):
        ...
        if name in StateUpdateMethod.stateupdaters:
            raise ValueError(
                f"A stateupdater with the name '{name}' " f"has already been registered"
            )
        ...
        if isinstance(method, Iterable) and not isinstance(method, str):
            ...
            for one_method in method:
                try:
                    ...
                    raise TypeError(
                        f"Each element in the list of methods has "
            ...
            if group_name is not None:
                msg_text = (
                    f"No numerical integration method specified for group "
                    f"'{group_name}', using method '{the_method}' ({timing})."
                )
            else:
                msg_text = (
                    f"No numerical integration method specified, "
            ...
            if hasattr(method, "__call__"):
                ...
                raise TypeError(
                    f"method argument has to be a string, a "
