...
class Morphology(object, metaclass=abc.ABCMeta):
    ...
    def _replace_three_point_soma(compartment, all_compartments):
        ...
        if (
            ...
            if (
                ...
                raise ValueError(
                    f"Cannot replace '3-point-soma' by a single "
    ...
    def from_points(points, spherical_soma=True):
        ...
        for counter, point in enumerate(points):
            if len(point) != 7:
                raise ValueError(
                    f"Each point needs to be described by 7 values, got {len(point)} instead."
                )
    ...
    def from_swc_file(filename, spherical_soma=True):
        ...
        with open(filename, "r") as f:
            ...
            for line_no, line in enumerate(f):
                ...
                if len(splitted) != 7:
                    raise ValueError(
                        f"Each line of an SWC file has to contain "
        ...
        if length is not None:
            ...
            for name, value in [("x", x), ("y", y), ("z", z)]:
                if value is not None and (value.ndim != 1 or len(value) != n + 1):
                    raise TypeError(
                        f"'{name}' needs to be a 1-dimensional array "
                        f"of length {n + 1}."
                    )
            self._x = (
