            ...
            if isinstance(K, slice) and K == slice(None):
                ...
                if isinstance(K, (numbers.Integral, slice)):
                    ...
                    raise NotImplementedError(
                        "Indexing synapses with arrays not" "implemented yet"
                    )
...
class Synapses(Group):
    ...
    def __init__(
        ...
        if not isinstance(model, Equations):
            raise TypeError(
                f"model has to be a string or an Equations "
        ...
        if isinstance(delay, Quantity):
            ...
            raise TypeError(
                f"Delay argument has to be a quantity or a "
        ...
        for pathway in delay:
            if not pathway in self._synaptic_updaters:
                raise ValueError(
                    f"Cannot set the delay for pathway "
    ...
    def _add_updater(self, code, prepost, objname=None, delay=None, event="spike"):
        ...
        if prepost == "pre":
            ...
            raise AssertionError(
                f"'prepost' argument has to be 'pre' or " f"'post', is '{prepost}'."
            )
        ...
        try:  # wrap everything to catch IndexError
            ...
            if condition is not None:
                if i is not None or j is not None:
                    raise ValueError(
                        "Cannot combine condition with i or j " "arguments"
                    )
            ...
            # 2: connection indices
                ...
                if skip_if_invalid:
                    raise ValueError(
                        "Can only use skip_if_invalid with string " "syntax"
                    )
    ...
    def _verify_connect_array_arguments(self, i, j, n):
        ...
        if not np.issubdtype(i.dtype, np.signedinteger):
            raise TypeError(
                f"Presynaptic indices have to be given as "
    ...
    def _condition_to_generator_expression(self, condition, p, namespace):
        ...
        if not is_boolean_expression(condition, variables):
            raise TypeError(f"Condition '{condition}' is not a " f"boolean condition")
        ...
        if not isinstance(p, str) and p == 1:
            j = f"_k for _k in range(N_post) " f"if {condition}"
        else:
            j = None
            if isinstance(p, str):
                ...
                if "_postsynaptic_idx" in p_dep or "_iterator_idx" in p_dep:
                    j = (
                        f"_k for _k in range(N_post) "
                        f"if ({condition}) and "
                        f"rand()<{p}"
                    )
            if j is None:
                j = f"_k for _k in sample(N_post, p={p}) " f"if {condition}"
        return j
    ...
    def _verify_connect_argument_types(self, condition, i, j, n, p):
        ...
        if isinstance(condition, str) and re.search(r"\bfor\b", condition):
            raise ValueError(
                f"Generator expression given for condition, write "
    ...
    def _resize(self, number):
        if not isinstance(number, (numbers.Integral, np.integer)):
            raise TypeError(
                f"Expected an integer number, got {type(number)} " f"instead."
            )
        if number < self.N:
            raise ValueError(
                f"Cannot reduce number of synapses, " f"{number} < {len(self)}."
            )
    ...
    def _add_synapses_from_arrays(self, sources, targets, n, p, namespace=None):
        ...
        try:
            ...
            logger.warn(
                "Cannot check whether the indices given for the connect call are "
    ...
    def _add_synapses_generator(
        ...
        for var in ["N_incoming", "N_outgoing"]:
            if var in identifiers:
                raise ValueError(f"The connect statement cannot refer to " f"'{var}'.")
