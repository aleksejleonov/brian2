...
def parse_synapse_generator(expr):
    ...
    if _cname(target) != "Name":
        raise SyntaxError(
            f"{parse_error} Generator must iterate over a single variable (not tuple, etc.)."
        )
    inner_variable = target.id
    iterator = generator.iter
    if _cname(iterator) != "Call" or _cname(iterator.func) != "Name":
        raise SyntaxError(
            parse_error + " Iterator expression must be one of "
            "the supported functions: " + str(list(iterator_function_handlers))
        )
