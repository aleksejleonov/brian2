...
def check_identifier_basic(identifier):
    ...
    if keyword.iskeyword(identifier):
        raise SyntaxError(
            f"'{identifier}' is a Python keyword and cannot be used as " f"a variable."
        )
...
def check_identifier_units(identifier):
    ...
    if identifier in DEFAULT_UNITS:
        raise SyntaxError(
            f"'{identifier}' is the name of a unit, cannot be used as a "
            f"variable name."
        )
...
def dimensions_and_type_from_string(unit_string):
    ...
    if unit_string == "bool":
        raise TypeError(
            "Use 'boolean' not 'bool' as the unit for a boolean " "variable."
        )
    ...
    for identifier in identifiers:
        if identifier not in _base_units:
            if identifier in DEFAULT_UNITS:
                ...
                raise ValueError(
                    f"Unit specification refers to "
    ...
    try:
        ...
        raise ValueError(
            f"Could not interpret '{unit_string}' as a unit specification: " f"{ex}"
        )
...
def parse_string_equations(eqns):
    ...
    for eq in parsed:
        ...
        try:
            ...
            raise EquationError(
                f"Error parsing the unit specification for "
        ...
        if isinstance(eqns, str):
            ...
            for eq in eqns:
                if not isinstance(eq, SingleEquation):
                    raise TypeError(
                        f"The list should only contain "
