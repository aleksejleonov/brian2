...
def _conflict_warning(message, resolutions):
    ...
    if len(resolutions) == 0:
        ...
        second_part = (
            f"but the name also refers to a variable in the "
            f"{resolutions[0][0]} "
            f"namespace with value '{_display_value(resolutions[0][1])}'."
        )
    else:
        second_part = (
            f"but the name also refers to a variable in the following "
...
def get_dtype(equation, dtype=None):
    ...
    if isinstance(dtype, Mapping):
        if equation.varname in dtype:
            ...
            if not provided_dtype.kind in BASIC_TYPES[equation.var_type]:
                raise TypeError(
                    f"Error determining dtype for variable "
    ...
    if equation.var_type == BOOLEAN:
        ...
        if dtype is not None:
            ...
            if not dtype.kind == "f":
                raise TypeError(f"{dtype} is not a valid floating point " f"dtype.")
            return dtype
        else:
            return prefs["core.default_float_dtype"]
    else:
        raise ValueError(
            f"Do not know how to determine a dtype for "
...
class Indexing(object):
    ...
    def __call__(self, item=slice(None), index_var=None):
        ...
        if isinstance(item, tuple):
            raise IndexError(
                f"Can only interpret 1-d indices, " f"got {len(item)} dimensions."
            )
        else:
            ...
            if isinstance(item, slice):
                ...
                if index_array.dtype == bool:
                    ...
                    raise TypeError(
                        f"Indexing is only supported for integer "
...
class VariableOwner(Nameable):
    ...
    def add_attribute(self, name):
        ...
        if name in self.variables:
            raise AttributeError(
                f'Cannot add an attribute "{name}", it is already a state variable of this group.'
            )
        if hasattr(self, name):
            raise AttributeError(
                f'Cannot add an attribute "{name}", it is already an attribute of this group.'
            )
    ...
    def _check_expression_scalar(self, expr, varname, level=0, run_namespace=None):
        ...
        for ref_varname, ref_var in referred_variables.items():
            if not getattr(ref_var, "scalar", False):
                raise ValueError(
                    f"String expression for setting scalar "
...
class Group(VariableOwner, BrianObject):
    ...
    def _resolve_external(
        ...
        if internal_variable is not None and user_identifier:
            ...
            if len(filtered_matches) == 0:
                ...
                if len(matches) == 1:
                    ...
                    warning_message += (
                        f"{namespace} namespace with the value " f"{value}. "
                    )
                else:
                    namespaces_list = " ,".join(match[0] for match in filtered_matches)
                    warning_message += f"following namespaces: " f"{namespaces_list}. "
        ...
        if not isinstance(resolved, (Function, Variable)):
            ...
            if value.shape != ():
                raise KeyError(
                    f"Variable {identifier} was found in the namespace, but is not a scalar value"
                )
