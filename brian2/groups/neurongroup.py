    ...
    def _get_refractory_code(self, run_namespace):
        ...
        if ref is False:
            ...
            if dims is second.dim:
                ...
                raise TypeError(
                    f"Refractory expression has to evaluate to a "
...
class Thresholder(CodeRunner):
    ...
    def update_abstract_code(self, run_namespace):
        ...
        if not is_boolean_expression(code, variables):
            raise TypeError(
                f"Threshold condition '{code}' is not a boolean " f"expression"
            )
...
class NeuronGroup(Group, SpikeSource):
    ...
    def __init__(
        ...
        logger.diagnostic(
            f"Creating NeuronGroup of size {self._N}, " f"equations {self.equations}."
        )
        ...
        for event_name in events.keys():
            if not isinstance(event_name, str):
                raise TypeError(
                    f"Keys in the 'events' dictionary have to be "
                    f"strings, not type {event_name}."
                )
            if not _valid_event_name(event_name):
                raise TypeError(
                    f"The name '{event_name}' cannot be used as an event " f"name."
                )
    ...
    def state(self, name, use_units=True, level=0):
        try:
            ...
            if name in self._linked_variables:
                raise TypeError(
                    f"Link target for variable {name} has not been " f"set."
                )
            else:
                raise ex
    ...
    def __setattr__(self, key, value):
        ...
        if not hasattr(self, "_group_attribute_access_active") or key in self.__dict__:
            ...
            log_msg = f"Setting {self.name}.{key} as a link to " f"{source}.{sourcevar}"
    ...
    def _repr_html_(self):
        ...
        def add_event_to_text(event):
            ...
            text.append(
                rf'<b>{event_header}:</b><ul style="list-style-type: none; margin-top: 0px;">'
            )
