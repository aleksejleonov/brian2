    ...
    *Casting rules*
    ...
    def __array_prepare__(self, array, context=None):
        ...
        if uf.__name__ in (
            ...
            fail_for_dimension_mismatch(
                args[0],
                error_message=(
                    "%s expects a " "dimensionless argument " "but got " "{value}"
                )
                % uf.__name__,
                value=args[0],
            )
    ...
    def _binary_operation(
        ...
        if fail_for_mismatch:
            if inplace:
                message = (
                    "Cannot calculate ... %s {value}, units do not " "match"
                ) % operator_str
    ...
    def _latex(self, expr):
        ...
        if unitless.ndim == 0:
            ...
            raise NotImplementedError(
                f"Cannot create a LaTeX representation for a "
                ...
                if (
                    ...
                    try:
                        ...
                        if have_same_dimensions(au[n], 1):
                            raise TypeError(
                                f"Argument {n} is not a unitless " f"value/array."
                            )
                        else:
                            raise TypeError(
            ...
            if "result" in au:
                ...
                if au["result"] == bool:
                    if not isinstance(result, bool):
                        error_message = (
                            f"The return value of function "
