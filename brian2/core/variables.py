...
class ArrayVariable(Variable):
    ...
    def __init__(
        ...
        if scalar and size != 1:
            raise ValueError(
                f"Scalar variables need to have size 1, not " f"size {size}."
            )
    ...
    def set_conditional_write(self, var):
        if not var.is_boolean:
            raise TypeError(
                f"A variable can only be conditionally writeable "
...
class VariableView(object):
        ...
        if isinstance(item, str):
            ...
            try:
                ...
                raise ReferenceError(
                    f"Cannot use string expressions, the "
                ...
                for index_group in index_groups:
                    ...
                    if (
                        ...
                        if not indexed_var is var:
                            logger.warn(
                                f"The string expression used for setting "
    ...
    def set_with_expression_conditional(
        ...
        if variable.scalar and cond != "True":
            raise IndexError(
                f"Cannot conditionally set the scalar variable " f"'{self.name}'."
            )
    ...
    def set_with_index_array(self, item, value, check_units):
        ...
        if variable.scalar:
            if not (isinstance(item, slice) and item == slice(None)):
                raise IndexError(
                    "Illegal index for variable '{self.name}', it is a "
                    "scalar variable."
                )
            indices = 0
        elif (
            ...
            if len(q.shape):
                if not len(q.shape) == 1 or len(q) != 1 and len(q) != len(indices):
                    raise ValueError(
                        f"Provided values do not match the size "
                        f"of the indices, "
                        f"{len(q)} != {len(indices)}."
                    )
    ...
    def __repr__(self):
        ...
        if self.variable.scalar:
            ...
            try:
                ...
                values = (
                    f"[Subexpression refers to external parameters. Use "
...
class Variables(Mapping):
    ...
    def _add_variable(self, name, var, index=None):
        if name in self._variables:
            raise KeyError(
                f"The name '{name}' is already present in the variables" f" dictionary."
            )
    ...
    def add_array(
        ...
        if values is not None:
            if scalar:
                ...
                if len(values) != size:
                    raise ValueError(
                        f"Size of the provided values does not match "
    ...
    def add_reference(self, name, group, varname=None, index=None):
        ...
        if varname not in group.variables:
            raise KeyError(
                f"Group {group.name} does not have a variable " f"{varname}."
            )
        if index is None:
