...
class TextReport(object):
    ...
    def __call__(self, elapsed, completed, start, duration):
        if completed == 0.0:
            self.stream.write(
                f"Starting simulation at t={start} for a duration of " f"{duration}\n"
            )
        else:
            t = str(completed * duration)
            percent = int(completed * 100.0)
            real_t = _format_time(float(elapsed))
            report_msg = f"{t} ({percent}%) simulated in " f"{real_t}"
            if completed < 1.0:
                remaining = int(round((1 - completed) / completed * float(elapsed)))
                remaining_msg = (
                    f", estimated {_format_time(remaining)} " f"remaining.\n"
                )
            else:
                remaining_msg = "\n"
        ...
        if isinstance(obj, SummedVariableUpdater):
            if obj.target_var in summed_targets:
                ...
                if obj.target == other_target:
                    ...
                    msg = (
                        f"Multiple 'summed variables' target the "
...
class Network(Nameable):
    ...
    def __getitem__(self, item):
        if not isinstance(item, str):
            raise TypeError(
                f"Need a name to access objects in a Network, "
    ...
    def restore(self, name="default", filename=None, restore_random_state=False):
        ...
        for obj in all_objects:
            if obj.name in state:
                ...
                raise KeyError(
                    f"Stored state does not have a stored state for "
        ...
        if len(unnused):
            raise KeyError(
                f"The stored state contains the state of the "
    ...
    def before_run(self, run_namespace):
        ...
        if len(non_unique_names):
            ...
            raise ValueError(
                f"All objects in a network need to have unique "
        ...
        logger.debug(
            f"Network '{self.name}' uses {len(self._clocks)} " f"clocks: {clocknames}",
            "before_run",
        )
    ...
    def run(
        ...
        if report is not None:
            ...
            if report == "text" or report == "stdout":
                ...
                raise TypeError(
                    f"Do not know how to handle report argument, "
