...
def plot_tau(ax, parameters):
    ...
    tau_group = NeuronGroup(
        100,
        Equations("v : volt")
        + reduce(
            operator.add,
            [construct_gating_variable_tau_equation(gv) for gv in ["m", "n", "h"]],
        ),
        method="euler",
...
def plot_inf(ax, parameters):
    ...
    inf_group = NeuronGroup(
        100,
        Equations("v : volt")
        + reduce(
            operator.add,
            [construct_gating_variable_inf_equation(gv) for gv in ["m", "n", "h"]],
        ),
        method="euler",
        namespace=parameters,
    )
...
def plot_conductances(ax, statemon):
    ...
    ax.plot(
        statemon.t / ms,
        statemon.g_K[0] / (mS / (cmeter**2)),
        label=r"$g_\mathregular{K}$",
    )

    ax.plot(
        statemon.t / ms,
        statemon.g_Na[0] / (mS / (cmeter**2)),
...
def plot_currents(ax, statemon):
    ...
    ax.plot(
        statemon.t / ms,
        statemon.I_Na[0] / (uA / (cmeter**2)),
        label=r"$I_\mathregular{Na}$",
    )

    ax.plot(
        statemon.t / ms,
        (statemon.I_Na[0] + statemon.I_K[0] + statemon.I_L[0]) / (uA / (cmeter**2)),
...
def plot_current_stimulus(ax, statemon):
    ...
    ax.plot(
        statemon.t / ms,
        statemon.I_ext[0] / (uA / (cmeter**2)),
        label=r"$I_\mathregular{ext}$",
    )
...
def run_simulation(parameters):
    ...
    for gating_variable in ["m", "n", "h"]:
        equations.append(construct_gating_variable_inf_equation(gating_variable))
        equations.append(construct_gating_variable_tau_equation(gating_variable))
...
parameters = {
    ...
    # for 0.5 ms each
    "I_stim": TimedArray(
        values=([0] * 4 + [4] + [0] * 15 + [15] + [0]) * uA / (cmeter**2), dt=0.5 * ms
    ),
