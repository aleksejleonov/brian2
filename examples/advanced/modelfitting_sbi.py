...
def simulate(params, I=1 * nA, t_on=50 * ms, t_total=350 * ms):
    ...
    spike_mon = SpikeMonitor(
        neurons, record=False, name="spike_mon"
    )  # record=False → do not record times
    ...
    return dict(v=Vm_mon.Vm, t=Vm_mon.t, I_inj=I_inj, spike_count=spike_mon.count)
...
if __name__ == "__main__":
    ...
    prior_min = [0.5, 1e-4]  # (gNa, gK) in µS
    prior_max = [80.0, 15.0]
    ...
    inference = sbi.inference.SNPE(prior, density_estimator=density_estimator_build_fun)
    ...
    posterior_sample = posterior.sample((1,), x=xo, show_progress_bars=False).numpy()
