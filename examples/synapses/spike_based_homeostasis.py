...
varying_stimulus = TimedArray(
    [25 * Hz, 50 * Hz, 0 * Hz, 35 * Hz, 0 * Hz], dt=10 * second
)
...
neuron_with_homeostasis = NeuronGroup(
    1,
    eqs,
    threshold="v > 1",
    reset="v = -1",
...
neuron_without_homeostasis = NeuronGroup(
    1,
    eqs,
    threshold="v > 1",
    reset="v = -1",
...
non_plastic_synapse_neuron_without_homeostasis = Synapses(
    varying_poisson, neuron_without_homeostasis, "w : 1", on_pre="v_post += w"
)
non_plastic_synapse_neuron_without_homeostasis.connect()
non_plastic_synapse_neuron_without_homeostasis.w = 2

non_plastic_synapse_neuron = Synapses(
    varying_poisson, neuron_with_homeostasis, "w : 1", on_pre="v_post += w"
)
...
# dt is in second
dts = np.arange(
    0.0, len(varying_stimulus.values) * varying_stimulus.dt, varying_stimulus.dt
)
x = list(itertools.chain(*zip(dts, dts)))
y = list(
    itertools.chain(*zip(varying_stimulus.values / Hz, varying_stimulus.values / Hz))
)
