...
S_stochastic.connect(
    "i != j", p="1.5 * exp(-((x_pre-x_post)**2 + (y_pre-y_post)**2)/(2*(60*umeter)**2))"
)
...
for color in ["g", "b", "m"]:
    ...
    plot(G.x[neuron_idx] / umeter, G.y[neuron_idx] / umeter, "o", mec=color, mfc="none")
    plot(
        G.x[S_deterministic.j[neuron_idx, :]] / umeter,
        G.y[S_deterministic.j[neuron_idx, :]] / umeter,
        color + ".",
    )
    subplot(1, 2, 2)
    plot(G.x[neuron_idx] / umeter, G.y[neuron_idx] / umeter, "o", mec=color, mfc="none")
    plot(
        G.x[S_stochastic.j[neuron_idx, :]] / umeter,
        G.y[S_stochastic.j[neuron_idx, :]] / umeter,
        color + ".",
    )

for idx, t in enumerate(["determininstic connections", "random connections"]):
    subplot(1, 2, idx + 1)
