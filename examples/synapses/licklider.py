...
"""
receptors = NeuronGroup(
    2, eqs_ear, threshold="x>1", reset="x=0", refractory=2 * ms, method="euler"
)
...
neurons = NeuronGroup(
    num_neurons, eqs_neurons, threshold="v>1", reset="v=0", method="euler"
)

synapses = Synapses(receptors, neurons, on_pre="v += 0.5")
synapses.connect()
synapses.delay = (
    "i*1.0/exp(log(min_freq/Hz)+(j*1.0/(num_neurons-1))*log(max_freq/min_freq))*second"
)
...
xlabel("Time (ms)")
ylabel("Frequency")
yticks([0, 99, 199, 299], array(1.0 / synapses.delay[1, [0, 99, 199, 299]], dtype=int))
show()
