...
S = Synapses(
    neuron,
    lfp,
    model="""w : ohm*meter**2 (constant) # Weight in the LFP calculation
                                   v_post = w*(Ic_pre-Im_pre) : volt (summed)""",
)
S.summed_updaters[
    "v_post"
].when = (
    "after_groups"  # otherwise Ic has not yet been updated for the current time step.
)
S.connect()
S.w = (
    "area_pre/(4*pi*sigma)/((x_pre-x_post)**2+(y_pre-y_post)**2+(z_pre-z_post)**2)**.5"
)
