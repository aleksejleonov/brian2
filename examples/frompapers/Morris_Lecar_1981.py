...
fig, (ax1, ax2) = plt.subplots(
    1, 2, gridspec_kw={"right": 0.95, "bottom": 0.15}, figsize=(6.4, 3.2)
)
...
for line_no, (idx, color) in enumerate(
    [(0, "C0"), (4, "C1"), (8, "C4"), (12, "C2"), (16, "C5")]
):
    n_null = (
        g_L * (V - V_L)
        + g_Ca * 0.5 * (1 + tanh((V - V_1) / V_2)) * (V - V_Ca)
        - neuron.I[idx]
    ) / (-g_K * (V - V_K))
    ax2.plot(V / mV, n_null, color=color)
    ax2.text(
        V[20 + 5 * line_no] / mV,
        n_null[20 + 5 * line_no] + 0.01,
        "%.0f" % (neuron.I[idx] / uA),
        color=color,
    )
# dn/dt nullcline
