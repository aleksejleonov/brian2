...
Y_T = 500.0 * mmolar  # Total vesicular neurotransmitter concentration
...
exc_syn.astrocyte_index = "int(x_post/grid_dist) + " "N_cols_a*int(y_post/grid_dist)"
### Astrocytes
...
ecs_astro_to_syn = Synapses(astrocytes, exc_syn, "G_A_post = G_A_pre : mmolar (summed)")
ecs_astro_to_syn.connect("i == astrocyte_index_post")
ecs_syn_to_astro = Synapses(
    exc_syn, astrocytes, "Y_S_post = Y_S_pre/N_incoming : mmolar (summed)"
)
...
astro_to_astro.connect(
    "i != j and " "sqrt((x_pre-x_post)**2 +" "     (y_pre-y_post)**2) < 75*um"
)
...
fig, ax = plt.subplots(
    ...
    ncols=1,
    sharex=True,
    figsize=(6.26894, 6.26894 * 0.8),
    gridspec_kw={"height_ratios": [1, 6, 2], "left": 0.12, "top": 0.97},
)
...
fraction = 4
ax[1].plot(
    exc_mon.t[exc_mon.i <= N_e // fraction] / second,
    exc_mon.i[exc_mon.i <= N_e // fraction],
    "|",
    color="C0",
)
ax[1].plot(
    inh_mon.t[inh_mon.i <= N_i // fraction] / second,
    inh_mon.i[inh_mon.i <= N_i // fraction] + N_e // fraction,
    "|",
    color="C1",
)
ax[1].plot(
    ast_mon.t[ast_mon.i <= N_a // fraction] / second,
...
spk_count, bin_edges = np.histogram(
    np.r_[exc_mon.t / second, inh_mon.t / second], int(duration / bin_size)
)
