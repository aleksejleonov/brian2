...
synapse_mon = StateMonitor(
    exc_syn, ["u_S", "x_S"], record=exc_syn[ni, :], when="after_synapses"
)
...
ax[0].plot(
    exc_mon.t[exc_mon.i <= N_e // 4] / ms,
    exc_mon.i[exc_mon.i <= N_e // 4],
    "|",
    color="C0",
)
ax[0].plot(
    inh_mon.t[inh_mon.i <= N_i // 4] / ms,
    inh_mon.i[inh_mon.i <= N_i // 4] + N_e // 4,
    "|",
    color="C1",
)
pu.adjust_spines(ax[0], ["left"])
ax[0].set(xlim=(0.0, duration / ms), ylim=(0, (N_e + N_i) // 4), ylabel="neuron index")

# Generate frequencies
bin_size = 1 * ms
spk_count, bin_edges = np.histogram(
    np.r_[exc_mon.t / ms, inh_mon.t / ms], int(duration / ms)
)
rate = double(spk_count) / (N_e + N_i) / bin_size / Hz
ax[1].plot(bin_edges[:-1], rate, "-", color="k")
pu.adjust_spines(ax[1], ["left", "bottom"])
ax[1].set(
    xlim=(0.0, duration / ms), ylim=(0, 10.0), xlabel="time (ms)", ylabel="rate (Hz)"
)
...
fig2, ax = plt.subplots(
    4,
    sharex=False,
    gridspec_kw={"left": 0.27, "bottom": 0.18, "top": 0.95, "hspace": 0.2},
    figsize=(3.07, 3.07),
)
...
ax[0].plot(
    [state_mon.t[0] / ms, state_mon.t[-1] / ms], [0, 0], color="grey", linestyle=":"
)
# Adjust axis
pu.adjust_spines(ax[0], ["left"])
ax[0].set(
    xlim=(0.0, duration / ms),
    ylim=(-5.0, 0.25),
    ylabel=f"postsyn.\nconduct.\n(${sympy.latex(nS)}$)",
)
...
ax[2].plot(
    synapse_mon.t[spk_index] / ms, synapse_mon.x_S[0][spk_index], ".", ms=4, color="C3"
)
ax[2].plot(
    synapse_mon.t[spk_index] / ms, synapse_mon.u_S[0][spk_index], ".", ms=4, color="C4"
)
...
ax[2].plot(
    synapse_mon.t / ms,
    1 + (synapse_mon.x_S[0] - 1) * exp(-(time - tspk) * Omega_d),
    "-",
    color="C3",
)
ax[2].plot(
    synapse_mon.t / ms,
    synapse_mon.u_S[0] * exp(-(time - tspk) * Omega_f),
    "-",
    color="C4",
)
# Adjust axis
