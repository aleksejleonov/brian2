...
syn_mon = StateMonitor(
    synapses, variables=["u_S", "x_S", "r_S", "Y_S"], record=[0, 1, 2]
)
...
fig, ax = plt.subplots(
    nrows=7,
    ncols=1,
    figsize=(6.26894, 6.26894 * 1.2),
    gridspec_kw={
        "height_ratios": [3, 2, 1, 1, 3, 3, 3],
        "top": 0.98,
        "bottom": 0.08,
        "left": 0.15,
        "right": 0.95,
...
ax[0].plot(
    np.asarray([-transient / second, 0.0]),
    np.asarray([C_Theta, C_Theta]) / umolar,
    ":",
    color="gray",
)
ax[0].set(
    xlim=[-transient / second, 0.0], yticks=[0.0, 0.4, 0.8, 1.2], ylabel=r"$C$ ($\mu$M)"
)
...
ax[6].set(
    xlim=(0, (duration - transient) / ms),
    ylim=(-5.0, 1500),
    xticks=np.arange(0, (duration - transient) / ms, 100),
    xlabel="time (ms)",
...
ax[6].legend(
    ["no gliotransmission", "weak gliotransmission", "stronger gliotransmission"],
    loc="upper right",
)
