...
source_neurons = NeuronGroup(
    1, "dx/dt = f_0 : 1", threshold="x>1", reset="x=0", method="euler"
)
## Dummy neuron
...
ecs_syn_to_astro = Synapses(
    synapses, astrocytes, "Y_S_post = Y_S_pre : mmolar (summed)"
)
ecs_syn_to_astro.connect()
################################################################################
# Monitors
################################################################################
astro_mon = StateMonitor(astrocytes, variables=["Gamma_A", "C", "h", "I"], record=True)
...
ax[1].set(
    xlim=(0.0, duration / second),
    ylim=[-0.1, 5.0],
    yticks=arange(0.0, 5.1, 1.0, dtype=float),
    ylabel=r"$I$ ($\mu M$)",
)
...
ax[2].set(xlim=(0.0, duration / second), ylim=[-0.1, 1.3], ylabel=r"$C$ ($\mu M$)")
pu.adjust_spines(ax[2], ["left"])

# Plot h
ax[3].plot(astro_mon.t / second, astro_mon.h.T)
ax[3].set(
    xlim=(0.0, duration / second), ylim=[0.4, 1.02], ylabel="h", xlabel="time ($s$)"
)
