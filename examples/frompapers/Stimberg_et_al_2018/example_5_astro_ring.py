...
# astrocytes
astro_to_astro.connect("j == (i + 1) % N_pre or " "j == (i + N_pre - 1) % N_pre")
...
ax.plot(
    astro_mon.t / second,
    (
        astro_mon.C[N_astro // 2 :].T / astro_mon.C.max()
...
ax.plot(
    astro_mon.t / second,
    (
        astro_mon.C[N_astro // 2 - 1].T / astro_mon.C.max()
...
ax.set(
    ...
    ylim=(0, (N_astro + 1.5) * scaling),
    xticks=np.arange(0.0, duration / second, 500),
    xlabel="time (s)",
