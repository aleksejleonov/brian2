...
V_thresh = -55.0 * mV  # spiking threshold
...
gleak = 30.0 * nS  # leak conductance
C = 300.0 * pF  # membrane capacitance
tau_AMPA = 2.0 * ms  # AMPA synaptic timeconstant
E_AMPA = 0.0 * mV  # reversal potential AMPA

ampa_max_cond = 5.0e-8 * siemens  # Ampa maximal conductance
w_max = 1.0  # maximal ampa weight
...
defaultclock.dt = 500.0 * us  # timestep
...
final_t = 20.0 * second  # end of simulation
input_time = 100.0 * ms  # duration of an input

#### Create neuron objects

Nrn_downstream = NeuronGroup(
    Nr_neurons,
    eqs_neurons,
...
Syn = Synapses(
    Nrns_input, Nrn_downstream, model=Syn_model, on_pre=Pre_eq, on_post=Post_eq
)
...
Nrn_downstream.I_ext = 0.0 * amp
Nrn_downstream.x_trace = 0.0
Nrns_input.v = V_rest
Nrns_input.x_trace = 0.0
