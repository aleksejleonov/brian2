...
g_AMPA_rec_E = 0.104 * nS * 800.0 / N_E
g_AMPA_ext_I = 1.62 * nS
g_AMPA_rec_I = 0.081 * nS * 800.0 / N_E
tau_AMPA = 2.0 * ms

# NMDA (excitatory)
g_NMDA_E = 0.327 * nS * 800.0 / N_E
g_NMDA_I = 0.258 * nS * 800.0 / N_E
...
g_GABA_E = 1.25 * nS * 200.0 / N_I
g_GABA_I = 0.973 * nS * 200.0 / N_I
tau_GABA = 10.0 * ms
...
input_reset_I = PoissonInput(
    P_E, "s_AMPA_ext", C_ext, rate_selection, "stimuli_reset(t)"
)
input_reset_E = PoissonInput(
    P_I, "s_AMPA_ext", C_ext, rate_selection, "stimuli_reset(t)"
)

# monitors
N_activity_plot = 15
sp_E_sels = [
    SpikeMonitor(P_E[pi : pi + N_activity_plot])
    for pi in range(N_non, N_non + p * N_sub, N_sub)
]
...
r_E_sels = [
    PopulationRateMonitor(P_E[pi : pi + N_sub])
    for pi in range(N_non, N_non + p * N_sub, N_sub)
]
...
for i, r_E_sel in enumerate(r_E_sels[::-1]):
    plot(
        r_E_sel.t / ms,
        r_E_sel.smooth_rate(width=25 * ms) / Hz,
...
for i, sp_E_sel in enumerate(sp_E_sels[::-1]):
    plot(
        sp_E_sel.t / ms,
        sp_E_sel.i + (p - i - 1) * N_activity_plot,
        ".",
        markersize=2,
        label=f"selective {p - i}",
    )
