...
V_thresh = -50.0 * mV  # spiking threshold
...
ampa_max_cond = 5.0e-10 * siemens  # Ampa maximal conductance
...
defaultclock.dt = 100.0 * us  # timestep
Nr_neurons = 2  # Number of neurons
rate_array = [1.0, 5.0, 10.0, 15.0, 20.0, 30.0, 50.0] * Hz  # Rates
...
Syn = Synapses(Nrns, Nrns, model=Syn_model, on_pre=Pre_eq, on_post=Post_eq)
...
for jj, rate in enumerate(rate_array):
    ...
    weight_result[0, jj] = 100.0 * Syn.w_ampa[0] / init_weight
    weight_result[1, jj] = 100.0 * Syn.w_ampa[1] / init_weight
