...
thresh_ex = (
    lambda s: optimize.fsolve(
        lambda th: (
            a * s * tauh * exp((Vi - th * volt) / (s * tauh))
            - th * volt * (1 - a)
            - a * (s * tauh + Vi)
            + vT
        )
...
ax1.vlines(S.t[S.i == 0] / ms, M.v[0, spike_timesteps] / mV, 0, color="r")
ax1.plot(S.t[S.i == 0] / ms, M.v[0, spike_timesteps] / mV, "ro", ms=3)
ax1.set(xlabel="Time (ms)", ylabel="Voltage (mV)", xlim=(0, 500), ylim=(-75, -35))
...
ax2.set(xlim=(0.5, 4), xlabel="Depolarization slope (mV/ms)", ylabel="Threshold (mV)")
