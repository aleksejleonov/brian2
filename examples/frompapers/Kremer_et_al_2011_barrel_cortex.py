...
Fmax = 0.5 / stim_change_time  # maximum firing rate in layer 4 (.5 spike / stimulation)
# Neuron parameters
...
layer4.selectivity = (
    "(i%N4)/(1.0*N4)*2*pi"  # for each barrel, selectivity between 0 and 2*pi
)
...
feedforward.connect(
    "(barrel_x_pre + barrelarraysize*barrel_y_pre) == barrel_idx_post", p=0.5
)
...
recurrent_exc = Synapses(
    layer23exc, layer23, model="w:volt", on_pre="ge+=w", name="recurrent_exc"
)
recurrent_exc.connect(p=".15*exp(-.5*(((x_pre-x_post)/.4)**2+((y_pre-y_post)/.4)**2))")
recurrent_exc.w["j<Nbarrels*N23exc"] = EPSC * 0.3  # excitatory->excitatory
...
recurrent_inh = Synapses(
    layer23inh, layer23exc, on_pre="gi+=IPSC", name="recurrent_inh"
)
...
selectivity_exc = (arctan2(_r, _i) % (2 * pi)) * 180.0 / pi


scatter(
    layer23.x[: Nbarrels * N23exc],
