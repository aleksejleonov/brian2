...
p3 = PoissonInput(
    group[1:], "ge", N=ne, rate=lambdae - (p * 1.0 / ne) * lambdac, weight=we
)
...
for i in [0, 1]:
    ...
    plot(
        tile(spikes, (2, 1)),
        vstack((val[array(spikes, dtype=int)], zeros(len(spikes)))),
        "C0",
    )
    title(
        "%s: %d spikes/second"
        % (["uncorrelated inputs", "correlated inputs"][i], M.count[i])
    )
tight_layout()
show()
