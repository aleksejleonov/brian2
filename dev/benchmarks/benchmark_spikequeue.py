...
def test_compress(N, create_func):
    ...
    results = timeit.repeat(
        "queue.compress()", ";".join(setup_code), repeat=5, number=number
    )
...
def test_push(N, create_func):
    ...
    number = 5000 / N
    results = timeit.repeat(
        "queue.push(np.arange({}));queue.next()".format(N),
        ";".join(setup_code),
        repeat=5,
        number=number,
    )
...
def run_benchmark(test_func, N, create_func):
    ...
    print(
        "{} -- {}({}) : {}".format(
            test_func.__name__, create_func, N, np.median(result)
        )
    )


if __name__ == "__main__":
    for test, N, create_func in itertools.product(
        (test_compress, test_push),
        (10, 100),
        ("create_all_to_all", "create_one_to_one"),
    ):
