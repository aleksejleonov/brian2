...
for t in threads:
    ...
    path = datapath + "_%d/" % t
    ids = numpy.fromfile(
        path + "results/_dynamic_array_spikemonitor_i", dtype=numpy.int32
    )
    times = numpy.fromfile(
        path + "results/_dynamic_array_spikemonitor_t", dtype=numpy.float64
    )
    w = numpy.fromfile(path + "results/_dynamic_array_synapses_w", dtype=numpy.float64)
    times_w = numpy.fromfile(
        path + "results/_dynamic_array_statemonitor_t", dtype=numpy.float64
    )
    w_over_time = numpy.fromfile(
        path + "results/_dynamic_array_statemonitor__recorded_w", dtype=numpy.float64
    )
    v_over_time = numpy.fromfile(
        path + "results/_dynamic_array_statemonitor_1__recorded_v", dtype=numpy.float64
    )
    times_v = numpy.fromfile(
        path + "results/_dynamic_array_statemonitor_1_t", dtype=numpy.float64
    )
    results[t]["spikes"] = (times, ids)
    results[t]["w"] = w
    results[t]["trace_w"] = w_over_time.reshape(
        len(times_w), len(w_over_time) / len(times_w)
    )
    results[t]["trace_v"] = v_over_time.reshape(
        len(times_v), len(v_over_time) / len(times_v)
    )
