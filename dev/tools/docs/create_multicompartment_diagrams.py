...
def to_svg(morphology):
    if isinstance(morphology, Soma):
        return (
            HEADER.format(minx="-5", miny="-5", width="10", height="10")
            + SPHERE.format(radius=morphology.diameter[0] / 2 / um)
            + FOOTER
        )
    if isinstance(morphology, Cylinder):
        ...
        if morphology.parent is not None:
            elements.append(
                PARENT_CYLINDER.format(
                    starty=(center - morphology.parent.end_diameter[-1] / 2) / um,
        ...
        for idx, (diameter, length) in enumerate(
            zip(morphology.diameter, morphology.length)
        ):
            elements.append(
                CONNECTION.format(
                    start=(summed_length - 3 * um) / um,
        ...
        return (
            HEADER.format(
                minx=minx,
                miny=-1,
                width=summed_length / um + 0.5 - minx,
        ...
        if morphology.parent is not None:
            ...
            minx -= morphology.parent.length[-1] / um
        for idx, (start_diameter, end_diameter, length) in enumerate(
            zip(morphology.start_diameter, morphology.end_diameter, morphology.length)
        ):
            elements.append(
                CONNECTION.format(
                    start=(summed_length - 3 * um) / um,
        ...
        return (
            HEADER.format(
                minx=minx,
                miny=-1,
                width=summed_length / um + 1.5 - minx,
                height=center * 2 / um + 2,
            )
            + ("\n".join(elements))
            + FOOTER
...
if __name__ == "__main__":
    ...
    root.sec = Section(
        n=5, diameter=[15, 5, 10, 5, 10, 5] * um, length=[10, 20, 5, 5, 10] * um
    )
    for filename, morpho in [
        ("soma.svg", Soma(diameter=30 * um)),
