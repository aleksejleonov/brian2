...
def plot_morphology2D(morpho, axis, color_switch=False):
    if isinstance(morpho, Soma):
        ...
        if color_switch:
            ...
            color = "darkred"
        ax.plot(coords[:, 0] / um, coords[:, 1] / um, color="black", lw=2)
        # dots at the center of the compartments
        ax.plot(morpho.x / um, morpho.y / um, "o", color=color, mec="none", alpha=0.75)
...
if __name__ == "__main__":
    ...
    for depth in range(1, 10):
        ...
        for endpoint in endpoints:
            new_L = Cylinder(
                n=max([1, int(length / (5 * um))]), diameter=diameter, length=length
            )
            new_R = Cylinder(
                n=max([1, int(length / (5 * um))]), diameter=diameter, length=length
            )
    ...
    for title, noise_sec, noise_comp in [
        ("section", 25, 0),
        ...
        for idx in range(3):
            ...
            print(idx)
            morpho_with_coords = morpho.generate_coordinates(
                section_randomness=noise_sec, compartment_randomness=noise_comp
            )
            ...
            mayavi.draw()
            mayavi.savefig(
                os.path.join(PATH, "morphology_random_%s_%d.png" % (title, idx + 1))
            )
            mayavi.close()
