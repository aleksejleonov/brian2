...
class ExampleRun(pytest.Item):
    ...
    def from_parent(cls, filename, codegen_target, dtype, parent):
        ...
        if hasattr(super_class, "from_parent"):
            new_node = super_class.from_parent(
                parent=parent, name=ExampleRun.id(filename)
            )
        else:
            # For pytest < 6
...
class ExampleCollector(pytest.Collector):
    ...
    def from_parent(cls, example_dir, parent):
        ...
        if hasattr(collector, "from_parent"):
            new_collector = collector.from_parent(parent, name="example_collector")
        else:
            # For pytest < 6
...
if __name__ == "__main__":
    ...
    sys.exit(pytest.main([__file__, "--verbose"], plugins=[Plugin(example_dir)]))
