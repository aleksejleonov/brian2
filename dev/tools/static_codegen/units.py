...
units_fname = os.path.normpath(
    os.path.join(curdir, "../../../brian2/units/allunits.py")
)
...
for longname, shortname, definition in derived_unit_table:
    derived += (
        '{longname} = Unit.create({definition}, "{longname}", "{shortname}")\n'.format(
