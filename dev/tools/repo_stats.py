...
try:
    ...
    raise ImportError(
        "Install PyGithub from https://github.com/PyGithub/PyGithub or via pip"
    )
API_TOKEN = None
...
for comment in comments:
    ...
    if name is None:
        authors.append(
            "`@{login} <https://github.com/{login}>`_".format(login=comment.user.login)
        )
    else:
        authors.append(
            "{name} (`@{login} <https://github.com/{login}>`_)".format(
                login=comment.user.login, name=name
            )
        )
    comment_counter += 1
...
for issue in issues:
    ...
    if name is None:
        authors.append(
            "`@{login} <https://github.com/{login}>`_".format(login=issue.user.login)
        )
    else:
        authors.append(
            "{name} (`@{login} <https://github.com/{login}>`_)".format(
                login=issue.user.login, name=name
            )
        )
    issue_counter += 1
